...
final class ServiceIPSumTest extends TestCase {
    ...
    public function ipDataProvider(): array {
        ...
        foreach (self::getTestData() as $ip => $data) {
            // case
            $data['threat'] = filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) ? 9 : 0;;
            $ipData[] = [
                ['ip' => $ip],
    ...
    public function ipListDataProvider(): array {
        ...
        foreach ($testData as $ip => $data) {
            $testData[$ip]['threat'] = filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV4) ? 9 : 0;
        }
