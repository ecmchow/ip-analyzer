...
declare -x STEPS=(
    ...
    'Service with SSL on TCP'
    'Service with Authentication and SSL'
    'Service with IPSum'
)
declare -x CMDS=(
    'testServiceBasic'
...
testServiceBasic () {
    ...
    php "${phpunit}" --testsuite service-basic || true
...
testServiceAuth () {
    ...
    php "${phpunit}" --testsuite service-auth || true
    result=$( tail -n 1 ${output} )
    php "${entry}" stop & sleep 0.1
    echo "${result}"
}
...
testServiceSSL () {
    if [ "$phar" = "yes" ]; then
        php "${entry}" start --basepath ../ --env test/env/auth-disabled-ssl.env & sleep 0.1
    else
        php "${entry}" start --env test/env/auth-disabled-ssl.env & sleep 0.1
    fi
    php "${phpunit}" --testsuite service-ssl || true
    result=$( tail -n 1 ${output} )
    php "${entry}" stop & sleep 0.1
    echo "${result}"
}
...
testServiceAuthSSL () {
    if [ "$phar" = "yes" ]; then
        php "${entry}" start --basepath ../ --env test/env/bcrypt.env & sleep 0.1
    else
        php "${entry}" start --env test/env/bcrypt.env & sleep 0.1
    fi
    php "${phpunit}" --testsuite service-auth-ssl || true
    result=$( tail -n 1 ${output} )
    php "${entry}" stop & sleep 0.1
    echo "${result}"
}
...
testServiceIPSum () {
    if [ "$phar" = "yes" ]; then
        php "${entry}" start --basepath ../ --env test/env/ipsum.env & sleep 0.1
    else
        php "${entry}" start --env test/env/ipsum.env & sleep 0.1
    fi
    php "${phpunit}" --testsuite service-ipsum || true
    result=$( tail -n 1 ${output} )
    php "${entry}" stop & sleep 0.1
    echo "${result}"
}
